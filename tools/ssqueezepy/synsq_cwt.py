...
def synsq_cwt_inv(Tx, fs, opts={}, Cs=None, freqband=None):  # TODO Arguments
    ...
    for n in range(Cs.shape[1]):
        TxMask = np.zeros(Tx.shape, dtype=Tx.dtype)
        UpperCs = np.minimum(np.maximum(Cs[:, n] + freqband[:, n], 1), len(fs))
        LowerCs = np.minimum(np.maximum(Cs[:, n] - freqband[:, n], 1), len(fs))
        ...
        for m in range(Tx.shape[1]):
            idxs = slice(int(LowerCs[m] - 1), int(UpperCs[m]))
