...
class CrossValAdapter:

    def __init__(self,data_path,fold,n_lead = 1):
        sample = self._get_signal(data_path,n_lead)
        train_sample,valid_sample,test_sample = self._gen_sample(sample,fold)
    ...
    def _gen_sample(self,res,fold,seed = 0,train_rate = 0.8):
        random.seed(seed)
        ...
        folds = 5 # 五折交叉检验
        if fold > folds:
            raise Exception("请检查fold，超出总数")
        fold_step = 105//folds
        test_index = index[fold_step*fold:fold_step*(fold + 1)]
        tmp_index = list(set(index) - set(test_index))
        train_size = int(len(tmp_index) * train_rate)
        train_index = tmp_index[:train_size]
        valid_index = tmp_index[train_size:]
