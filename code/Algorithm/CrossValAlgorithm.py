...
from DataAdapter.CrossValAdapter import CrossValAdapter,TrainAdapter,ValidAdapter,TestAdapter
...
class Algorithm():
    ...
    def __init__(self,lead = 0,fold = 0,parallel = True):
        super(Algorithm,self).__init__()
        self.save_path = r'C:\Users\yurui\Desktop\item\cpsc\code\pretrain\model\CNN' # 保存训练好的模型文件的路径
        pretrain_model_path = r'C:\Users\yurui\Desktop\item\cpsc\code\pretrain\model\pretrain\pretrain_model' + str(lead) + '.pt' # 加载预训练模型文件的路径
        ...
        base_adapter = CrossValAdapter(data_path,fold,n_lead = self.lead) # 训练集的数据生成器
        ...
        self.early_stopping = EarlyStopping(self.save_path,lead = self.lead,patience = patience,verbose = False)
    ...
    def test(self):
        ...
        model.load_state_dict(torch.load(os.path.join(self.save_path,'CNN_model' + str(self.lead) +'.pt')))
        model.cuda()
        start_time = time.time()
...
class EarlyStopping:
    def __init__(self, save_path, lead, patience=7, verbose=False, delta=0):
        """
        Args:
    ...
    def save_checkpoint(self, val_loss, model):
        ...
        torch.save(model.state_dict(), os.path.join(self.save_path,'CNN_model' + str(self.lead) +'.pt'))
